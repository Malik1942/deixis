...
/// The part of GitHub's `releases/latest` reply that Deixis reads.
...
enum UpdateCheck {
    static let endpoint = URL(string: "https://api.github.com/repos/Malik1942/deixis/releases/latest")!
    ...
    /// The one request Deixis makes: the version number in the User-Agent, nothing else.
    static func request(current: AppVersion?) -> URLRequest {
        ...
        request.setValue("Deixis/\(current.map(\.description) ?? "unknown")", forHTTPHeaderField: "User-Agent")
        return request
    }
