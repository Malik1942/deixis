...
- Element quality depends on the target app's accessibility implementation. SwiftUI, AppKit, and the iOS Simulator work well. Electron, Figma, games, and custom-drawn UIs often expose little; Deixis then records `element: null`, says so in the payload, and still gives the agent the image and your note.
